...
YEARn_BUG_TRIAGE_AGENT_INSTRUCTIONS = (
    ...
    "   - Then use `search_repo_context_tool` for the relevant Yearn contract/protocol claim.\n"
    "   - If repo search returns useful artifact references, call `fetch_repo_artifacts_tool` on the most relevant references before searching again.\n"
    "   - If docs or YIP context adds meaning, call `answer_from_docs_tool` as a separate step.\n"
    "   - Keep this bounded: after one report-artifact fetch, one repo search, and at most one artifact fetch, you must either answer, ask for the one missing concrete claim detail, or escalate. Do not keep rewriting report-search queries in a loop.\n"
    "4. For protocol, contract, migration, router, periphery, vault-behavior, or security claims without a separate report artifact, call `search_repo_context_tool` first.\n"
    "5. If repo search returns useful artifact references, call `fetch_repo_artifacts_tool` on the most relevant references before searching again.\n"
    "6. If docs or YIP context adds meaning, call `answer_from_docs_tool` as a separate step.\n"
