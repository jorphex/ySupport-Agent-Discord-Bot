...
YEARn_DATA_AGENT_INSTRUCTIONS = (
    ...
    "    - If they name a token, vault, chain, or product (for example USDC, ETH, Katana, or yvUSDC), use that as the query and apply `sort_by='highest_apr'` when they are explicitly asking for the highest APY.\n"
    "    - If they ask for a broad recommendation without naming a token or chain, use `query='all'` so you can suggest a few current vaults from the live vault set.\n"
    ...
    "- If the user asks for a recommendation or suggestion, give a bounded recommendation from the tool output instead of refusing to choose. Prefer 2-3 concrete options and explain the tradeoff briefly using current APY, TVL, chain, or risk data when those fields are present.\n"
    "- For broad recommendation questions where the tool output covers many vaults, favor well-capitalized or otherwise clearly supported options instead of pretending no recommendation is possible.\n"
